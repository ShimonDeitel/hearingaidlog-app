...
struct ContentView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Hearing Aid Log")
            .toolbar {
    ...
    @ViewBuilder
    private func row(_ entry: EntryEntry) -> some View {
...
struct EntrySheet: View {
    ...
    @State private var tag: String = HearingAidLogTags.all.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Days left: \(Int(metric))") {
                    Slider(value: $metric, in: 0...10, step: 1)
                        .accessibilityIdentifier("entryMetricSlider")
                }
                Section("Device") {
                    Picker("Device", selection: $tag) {
                        ForEach(HearingAidLogTags.all, id: \.self) { t in
                            Text(t).tag(t)
                        }
                    }
                    .pickerStyle(.menu)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let t = title.isEmpty ? "Entry" : title
